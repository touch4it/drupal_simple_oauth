...
namespace Drupal\Tests\simple_oauth_3p\Functional;
...
/**
 * Class ImplicitFunctionalTest
 *
 * @package Drupal\Tests\simple_oauth_3p\Functional
 *
 * @group simple_oauth_3p
 */
class ImplicitFunctionalTest extends TokenBearerFunctionalTestBase {
  ...
  protected function setUp() {
    parent::setUp();
    $this->redirectUri = Url::fromRoute('oauth2_token_3p.test_token', [], [
      'absolute' => TRUE,
    ])->toString();
    $this->client->set('redirect', $this->redirectUri);
    $this->client->save();
    $this->authorizeUrl = Url::fromRoute('oauth2_token_3p.authorize');
  ...
  public function testImplicitGrant() {
    ...
    $this
      ->config('simple_oauth_3p.settings')
